...
def get_autosky_alerts(alert_ini, warnings_file, master_enable):
    ...
    no_alerts_text = '<span style=\'color: #FF0000;\'>No Alerts</span>'  # Bright red for no alerts
    ...
    if os.path.exists(warnings_file):
        ...
        if not alert_content:
            ...
            return f'"{enabled_text}<br><span style=\'color: #FF0000;\'>{alert_url_link}<b>{alert_content_cleaned}</b>{alert_url_link_end}</span>"' # Bright red for active alerts
    else:
        return '" "'
